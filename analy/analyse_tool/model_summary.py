...
def get_model_flops(model, input_res, coord, cell, print_per_layer_stat=True,
    ...
    if input_constructor:
        ...
        coord_ = torch.FloatTensor(1, *coord).to(device)
        cell_ = torch.FloatTensor(1, *cell).to(device)
        _ = flops_model(batch, coord_, cell_)
...
def get_model_activation(model, input_res, coord, cell, input_constructor=None):
    assert type(input_res) is tuple, 'Please provide the size of the input image.'
    assert len(input_res) >= 3, 'Input image should have 3 dimensions.'
    ...
    if input_constructor:
        ...
        coord_ = torch.FloatTensor(1, *coord).to(device)
        cell_ = torch.FloatTensor(1, *cell).to(device)
        _ = activation_model(batch, coord_, cell_)
