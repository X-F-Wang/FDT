...
def eval_psnr(
    ...
    elif eval_type.startswith('screen'):
        scale = int(eval_type.split('-')[1])
        metric_fn = partial(utils.calc_psnr, dataset='screen', scale=scale)
...
if __name__ == '__main__':
    ...
    save_path = os.path.join('./test', save_name)
    ...
    log_file = 'log.txt'  # 指定日志文件的文件名

    log_file_path = os.path.join(save_path, log_file)  # 创建完整的日志文件路径

    with open(os.path.join(save_path, 'config.yaml'), 'w') as f:
        yaml.dump(config_, f, sort_keys=False)

    with open(log_file_path, 'w') as f:
        log_info = ['result: {:.4f}'.format(res),
                    'consumed time: {}'.format(utils.time_text(t2 - t1)),
                    'memory: {:.0f}M'.format(torch.cuda.max_memory_allocated(torch.cuda.current_device()) / 1024 ** 2)]
        log_str = ', '.join(log_info)
        f.write(log_str)  # 将log_str写入文件