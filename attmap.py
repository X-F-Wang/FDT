...
#os.environ.update({"QT_QPA_PLATFORM_PLUGIN_PATH": "/home/AMsIT1//envs/research-headless/lib/python3.8/site-packages/PyQt5/Qt5/plugins/xcbglintegrations/libqxcb-glx-integration.so"})


# def parse_indices(img_path):
#     coord_indices = [255,255,255]
#
...
def visualize(model, inp, coords, cell, save_dir):
        ...
        for j in range(attn_maps[i].shape[0]):
            for k in range(attn_maps[i][j].shape[-1]):
                save_img(attn_maps[i][j, ..., k].cpu().numpy(), os.path.join(attn_dir, \
                                                                             'att_map_iter{}_idx{}_head{}.png'.format(i,
                                                                                                                      j,
                                                                                                                      k)))
...
if __name__ == '__main__':
    ...
    #pred = visualize(model, inp, coords, cell, save_dir, visual_indices)
    pred = visualize(model, inp, coords, cell, save_dir)
